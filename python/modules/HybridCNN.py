...
class HybridCNN(CommonModel):
    def __init__(self, alphasize, emb_dim, dropout=0.0, avg=0, cnn_dim=256, activation=nn.ReLU):
    ...
    def forward(self, x):
        # Reshape input from (batch_size, seq_len, alphasize) to (batch_size, alphasize, seq_len)
        x = x.permute(0, 2, 1)
